...
def import_checkpoint(file) -> Tuple[str, dict]:
    ...
    try:
        ...
        state.current_epoch = checkpoint.get('epoch', 0)
        state.total_steps = checkpoint.get('total_steps', 0)
        state.current_loss = checkpoint.get('loss', 0.0)

        # Lock config
        state.loaded_config = config
        state.config_locked = True

        log(f"Checkpoint loaded successfully", "INFO")
        ...
        log(f"  Resumed at step {state.current_step}", "INFO")
...
def training_worker(
    ...
    try:
        ...
        for chunk_idx, (chunk_texts, chunk_samples, chunk_bytes, total_bytes_so_far) in enumerate(chunk_generator):
            ...
            state.status = f"Writing chunk {chunk_idx + 1}..."
            ...
            state.status = f"Encoding chunk {chunk_idx + 1}..."
            ...
            # Cleanup tokens to free memory before next chunk
            del all_tokens
            del dataloader
            gc.collect()
