...
These tokens will appear in every training example, so the model will learn their meaning from context.

## Formatting Conversations

SFT data is just conversations formatted as text:
...
The cosine shape is smooth—no sudden changes that might destabilize training.
...
const model = new ChatGPT(vocabSize, embeddingDim, numHeads, numBlocks, maxSeqLength, optimizer);
